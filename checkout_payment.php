...
if (DISPLAY_CONDITIONS_ON_CHECKOUT == 'true') {
        ...
        $shop_content_query1 = xtc_db_query("SELECT
                                                content_title,
                                                content_heading,
                                                content_text,
                                                content_file
                                                FROM ".TABLE_CONTENT_MANAGER."
                                                WHERE content_group='11' ".$group_check."
                                                AND languages_id='".$_SESSION['languages_id']."'");
        $shop_content_data1 = xtc_db_fetch_array($shop_content_query1);

        if ($shop_content_data1['content_file'] != '') {

                $revocation = '<iframe SRC="'.DIR_WS_CATALOG.'media/content/'.$shop_content_data1['content_file'].'" width="100%" height="300">';
                $revocation .= '</iframe>';
        } else {

                $revocation = '<textarea name="blabla1" cols="60" rows="10" readonly="readonly">'.strip_tags(str_replace('<br />', "\n", $shop_content_data1['content_text'])).'</textarea>';
        }
