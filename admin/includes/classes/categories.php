...
/* --------------------------------------------------------------
   $Id: categories.php 25 2012-06-10 17:59:00Z deisold $
...
// ### MOD admin_specials - Modules
		...
		foreach($products_data['products_details_id'] as $Key => $Products_Details) {
			$Key_Temp = explode('.', $Key);

			$Temp_Sql = "SELECT * FROM ".TABLE_PRODUCTS_DETAILS_VALUES."
						WHERE products_details_id=".xtc_db_prepare_input($Key_Temp[0])."
							AND language_id=".xtc_db_prepare_input($Key_Temp[1])."
							AND products_id=".xtc_db_prepare_input($products_id).";";
			$Temp_Item = xtc_db_num_rows(mysql_query($Temp_Sql));
			if($Temp_Item > 0) {
				$Product_Details_Sql = "UPDATE ".TABLE_PRODUCTS_DETAILS_VALUES."
										SET products_details_value='".$products_data['products_details_id'][$Key]."'
										WHERE products_details_id=".xtc_db_prepare_input($Key_Temp[0])."
											AND language_id=".xtc_db_prepare_input($Key_Temp[1])."
											AND products_id=".xtc_db_prepare_input($products_id).";";
			}
			else {
				$Product_Details_Sql = "INSERT INTO ".TABLE_PRODUCTS_DETAILS_VALUES."
										SET products_details_value='".$products_data['products_details_id'][$Key]."', products_details_id=".xtc_db_prepare_input($Key_Temp[0]).", language_id=".xtc_db_prepare_input($Key_Temp[1]).", products_id=".xtc_db_prepare_input($products_id).";";
			}
			xtc_db_query($Product_Details_Sql);
		}
	} // insert_product ends
