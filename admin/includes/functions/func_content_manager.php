...
function content($lang, $box, $parent){
    ...
    while($content_data = xtc_db_fetch_array($sql_content)){
      ...
      if ($content_data['content_delete']=='1'){
        $delete = '<a href="'. xtc_href_link(FILENAME_CONTENT_MANAGER,'special=delete&coID='.$content_data['content_id']).'" onClick="return confirm(\''.$content_data['content_title'].'\n\n'. CONFIRM_DELETE.'\')">'
                    .xtc_image(DIR_WS_ICONS.'delete.gif','Delete','','','style="cursor:pointer"').'</a> ';
      }else{
        $delete = xtc_image(DIR_WS_ICONS . 'locked.gif');
      }
      $edit = '<a href="'.xtc_href_link(FILENAME_CONTENT_MANAGER,'action=edit&coID='.$content_data['content_id']).'">'.xtc_image(DIR_WS_ICONS.'icon_edit.gif','Edit','','','style="cursor:pointer"').'</a>';
      $preview = '<a style="cursor:pointer" onClick="javascript:window.open(\''.xtc_href_link(FILENAME_CONTENT_PREVIEW,'coID='.$content_data['content_id']).'\', \'popup\', \'toolbar=0, width=640, height=600\')">'.xtc_image(DIR_WS_ICONS.'preview.gif','Preview','','','style="cursor:pointer"').'</a>';
				if (GROUP_CHECK == 'true') {// symbole der gruppen ausgeben, für welche der content sichtbar ist
				  $group_check = '';
				  $grp_id = '';
