<?php
/* --------------------------------------------------------------
   $Id: customers.php 48 2012-09-17 12:49:26Z deisold $
...
?>
      <tr>
		...
		{ echo xtc_image(DIR_WS_ICONS . $customers_statuses_array[$customers['customers_status']]['csa_image'], ''); } ?></td>
            <td class="main"></td>
...
<!-- Kunden Buchstaben -->
<?php
	echo '<div class="navigationSelection"><ul class="navigationList">'.xtcIndexNavigation(FILENAME_CUSTOMERS).'</ul></div>';
?>
<!-- Kunden Buchstaben -->
...
<td class="dataTableHeadingContent"><?php echo TABLE_HEADING_CUSTOMERS_CID.xtc_sorting(FILENAME_CUSTOMERS, 'customers_cid'); ?></td>
...
<?php
	...
	if ($_GET['sorting_letter'])
	{
		...
		{
			$letter_sort = " and c.customers_lastname LIKE '" . xtc_db_prepare_input($_GET['sorting_letter']) . "%' ";
		} else
		{
			$letter_sort = '';
		}
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if ( (is_object($cInfo)) && ($customers['customers_id'] == $cInfo->customers_id) )
			{ echo xtc_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else
			{ echo '<a href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '">' . xtc_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
