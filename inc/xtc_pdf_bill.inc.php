...
function xtc_pdf_bill ($oID, $send=false, $deliverSlip=false) {
    ...
    $pdf->RechnungStart($order->customer['name'], $customer_gender, $deliverSlip);
    ...
    if ($deliverSlip == false) {
        ...
        while ($oder_total_values = xtc_db_fetch_array($resOrderTotal)) {
        	$Temp = explode(' ', $oder_total_values['title']);
        	if($oder_total_values['class'] == 'ot_shipping' && $Temp[0] == 'Versandkostenfrei') {
	        	$order_data[] = array (
	                'title' => $Temp[0].':',
