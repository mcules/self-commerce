<?php
    ...
    if (strpos($PHP_SELF, FILENAME_LOGOFF) === false) {
      ...
      for ($i = 0, $n = $sizeof_products; $i < $n; $i++) {
        ...
        $products_in_cart[] = array ('QTY' => $products[$i]['quantity'],
                                     ...
                                     'PRICE'=> $xtPrice->xtcFormat($products[$i]['price'] * $products[$i]['quantity'], true));
      }
    }
  ...
  $box_smarty->assign('TOTALNULL', $xtPrice->xtcFormat(0, true));
  ...
  if (isset($_SESSION['customer_id'])) {
    $gv_query = xtc_db_query("-- /templates/".CURRENT_TEMPLATE."/source/boxes/shopping_cart_header.php
                              SELECT amount
...
//  $box_smarty->assign('LINK_CART', xtc_href_link(FILENAME_SHOPPING_CART, '', 'NONSSL'));
  ...
  $box_shopping_cart = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_cart_header.html');
  $smarty->assign('box_CART_HEADER', $box_shopping_cart);
?>