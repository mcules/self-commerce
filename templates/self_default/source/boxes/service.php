...
$ers = array(
        ...
        $id_name = strtr($content_data['content_title'],$ers);
        $id_name = strtolower($id_name);
    $service_data[] = array('LINK' => xtc_href_link(FILENAME_POPUP_CONTENT, 'coID='.$content_data['content_group']),
                            'ID_NAME' => $id_name,
                            'NAME' => $content_data['content_title']);
	}

	if (!empty($service_data)) {
		$box_smarty->assign('service_data', $service_data);
	}
}
...
if (!$cache) {
    $box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_service.html');
} else {
    $box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_service.html', $cache_id);
}
//EOF - DokuMan - 2010-02-28 - fix Smarty cache error on unlink

$smarty->assign('box_SERVICE', $box_content);
?>