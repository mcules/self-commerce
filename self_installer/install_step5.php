					...
					<tr>
						<td>
							...
							if (!isset($_POST['LAST_UPDATE'])) {
							    $Last_Update = date('Y-m-d', time());
							    $This_Update = $Last_Update;
							}
							else {
							    $Last_Update = $_POST['LAST_UPDATE'];
							    $This_Update = date('Y-m-d', time());
							}

							if (!$db_error) { xtc_db_test_connection($db['DB_DATABASE']); }
							if ($db_error) {
							?>
							...
							<?php
							    ...
							    while (list($key, $value) = each($_POST)) {
									if ($key != 'x' && $key != 'y') {
										...
										else { echo xtc_draw_hidden_field_installer($key, $value); }
									}
							    }
							?>
...
<?php
    ...
    $file_contents = '<?php' . "\n" .
                     ...
                     '  (c) 2004-2880 XT:Commerce (configure.php); www.xtcommerce.com' . "\n" .
                     '  (c) 2008-'.date('Y', time()).' Self-Commerce (configure.php); www.self-commerce.de' . "\n" .
                     '' . "\n" .
                     '  Released under the GNU General Public License' . "\n" .
                     '  --------------------------------------------------------------*/' . "\n" .
                     '' . "\n" .
                     ...
                     'define(\'LAST_UPDATE\', \'' . $This_Update . '\'); // When was the last update' . "\n" .
                     '' . "\n" .
    ...
    $file_contents = '<?php' . "\n" .
                     ...
                     '  (c) 2000-2001 The Exchange Project  (earlier name of osCommerce)' . "\n" .
                     '  (c) 2002-2003 osCommerce (configure.php,v 1.13 2003/02/10); www.oscommerce.com' . "\n" .
                     '  (c) 2004-2008 XT:Commerce (configure.php); www.xtcommerce.com' . "\n" .
                     '  (c) 2008-'.date('Y', time()).' Self-Commerce (configure.php); www.self-commerce.de' . "\n" .
                     '' . "\n" .
                     '  Released under the GNU General Public License' . "\n" .
                     '  --------------------------------------------------------------*/' . "\n" .
                     ...
                     'define(\'DIR_FS_DOWNLOAD_PUBLIC\', DIR_FS_CATALOG . \'pub/\');' . "\n" .
                     'define(\'DIR_FS_INC\', DIR_FS_CATALOG . \'inc/\');' . "\n" .
                     'define(\'LAST_UPDATE\', \'' . $Last_Update . '\'); // When was the last update' . "\n" .
                     '' . "\n" .
                     '// define our database connection' . "\n" .
                     'define(\'DB_SERVER\', \'' . $_POST['DB_SERVER'] . '\'); // eg, localhost - should not be empty for productive servers' . "\n" .
                     'define(\'DB_SERVER_USERNAME\', \'' . $_POST['DB_SERVER_USERNAME'] . '\');' . "\n" .
