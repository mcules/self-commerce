...
/**
 * Usage
 * 	require_once('Class_Radius.php');
 * 	$Class_Radius = new Radius($ip_radius_server = 'radius_server_ip_address', $shared_secret = 'radius_shared_secret'[, $radius_suffix = 'optional_radius_suffix'[, $udp_timeout = udp_timeout_in_seconds[, $authentication_port = 1812]]]);
 * 	$result = $Class_Radius->Access_Request($username = 'username', $password = 'password'[, $udp_timeout = udp_timeout_in_seconds]);
 *
 * Examples
 *	Example 1
 *		<?php
 *         	require_once('Class_Radius.php');
 *         	$Class_Radius = new Radius('127.0.0.1', 'secret');
 *			// Needed for some devices, and not auto_detected if PHP not runned through a web server
 *          $Class_Radius->SetNasIpAddress('1.2.3.4');
 *          if ($Class_Radius->AccessRequest('user', 'pass')) {
 ...
 *             	echo "<br />";
 *         	}
 *         	echo $Class_Radius->GetReadableReceivedAttributes();
 *     	?>
 *********************************************************************/
...
class Radius {
    ...
    var $debug_messages;		  // Debug Messages
    ...
    function DebugInfo($debug_info) {
        if ($this->_debug_mode) {
            $this->debug_messages .= date('Y-m-d H:i:s').' DEBUG: ';
            $this->debug_messages .= $debug_info;
            $this->debug_messages .= '<br />';
            flush();
        }
    }
