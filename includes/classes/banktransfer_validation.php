<?php 
/* -----------------------------------------------------------------------------------------
   $Id: banktransfer_validation.php 2012-06-06 14:40:57 cr $   
...
/*                                                                               */
		...
		function is_valid_bic($bic) {
			
			$result = 0;
			
			if (!$this->bic_validate($bic)) {
				 $result = 1050;
			}
    ...
    function init() {
		...
		function make_iban($kontonr, $blz, $ctry_iso2 = 'DE') {
			
			// Standardmethode zur Erzeugung von IBAN-Nummern aus herkömmlichen Kontonummern und BLZ
			// Diese Methode ist mit Bedacht einzusetzen, da die 47 IBAN-Regeln der deutschen Kreditwirtschaft
			// nicht berücksichtigt sind. Die errechnete IBAN kann daher von der tatsächlichen IBAN abweichen. 
			// Mehr Infos zu den IBAN-Regeln (Ausnahmeregeln):
			// http://www.bundesbank.de/Navigation/DE/Kerngeschaeftsfelder/Unbarer_Zahlungsverkehr/IBAN_Regeln/iban_regeln.html
			
			// BLZ
			$checkblz = 0;
			$ctry_iso2 = strtoupper($ctry_iso2);
			if ($ctry_iso2 == 'DE') { 
			  $blz = str_pad ($blz, 8, "0", STR_PAD_RIGHT);
				$checkblz = $this->query($blz);
				$kontonr = $this->ExpandAccount($kontonr);
			}
