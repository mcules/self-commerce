...
/* -----------------------------------------------------------------------------------------
   $Id: product_reviews_info.php 17 2012-06-04 20:33:29Z deisold $   
...
$reviews_query = xtc_db_query("select rd.reviews_text, 
                              ...
                              left join ".TABLE_PRODUCTS_DESCRIPTION." pd on (p.products_id = pd.products_id and pd.language_id = '".(int) $_SESSION['languages_id']."'), ".TABLE_REVIEWS_DESCRIPTION." rd 
                              where r.reviews_id = '".(int) $_GET['reviews_id']."' and r.reviews_id = rd.reviews_id and p.products_status = '1'");
...
$smarty->assign('IMAGE', '<a href="'.DIR_WS_POPUP_IMAGES.$reviews['products_image'].'" rel="lightbox" title="'.$reviews['products_name'].'">'.xtc_image(DIR_WS_INFO_IMAGES.$reviews['products_image'], $reviews['products_name'], '', '', 'align="center" hspace="5" vspace="5"').'<br /></a>');
